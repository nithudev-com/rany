...
export default function EditProductForm({ product }: { product: any }) {
  ...
  const initialImages = Array.isArray(product.images) && product.images.length > 0 
    ? product.images.map((img: any) => ({
        ...img,
        secureUrl: img.secureUrl || img.imageUrl // Map database imageUrl to UI secureUrl
      }))
    : (product.mainImage ? [{ secureUrl: product.mainImage, isPrimary: true, publicId: 'legacy' }] : []);
  ...
  const [stockQuantity, setStockQuantity] = useState(product.stockQuantity !== null ? product.stockQuantity : '');
  const [lowStockLimit, setLowStockLimit] = useState(product.lowStockLimit || '');
  ...
  const [shippingClass, setShippingClass] = useState(product.shippingInfo?.class || '');

  // Complex Arrays
  const [details, setDetails] = useState<{key: string, value: string}[]>(Array.isArray(product.details) ? product.details : []);
  const [faqs, setFaqs] = useState<{question: string, answer: string}[]>(Array.isArray(product.faq) ? product.faq : []);
  
  // Variants
  const [variantAttributes, setVariantAttributes] = useState<{name: string, options: string}[]>([]);
  const [variants, setVariants] = useState<any[]>(Array.isArray(product.variants) ? product.variants : []);
  ...
  const handleSubmit = async (submitStatus: string) => {
    ...
    try {
      ...
      setErrorMessages([err.message || 'Server error updating product.']);
      window.scrollTo(0, 0);
      setLoading(false);
    }
  };
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      ...
      if (resDetails.success && resDetails.data) {
        ...
        if (resDetails.data.tags && Array.isArray(resDetails.data.tags)) {
           handleChange(setTags, resDetails.data.tags.join(', '));
        }
        if (resDetails.data.shortDescription && !shortDescription) {
           handleChange(setShortDescription, resDetails.data.shortDescription);
        }
      } else {
      ...
      // 5. Advanced SEO Keywords & Image Alts
      setAiProgress('Generating Advanced Focus Keyword & Image Alts...');
      ...
      const resKw = await executeAITask('seoKeywordsAndAlts', kwVars);
      if (resKw.success && resKw.data) {
        setAiProgress('Focus Keyword & Image Alts — Completed');
        if (resKw.data.focusKeyword) {
          handleChange(setFocusKeyword, resKw.data.focusKeyword);
        }
        
        if (imageCount > 0 && Array.isArray(resKw.data.imageAlts)) {
          const newImages = [...images];
          let changed = false;
          newImages.forEach((img, idx) => {
            img.altText = resKw.data.imageAlts[idx] || `${resKw.data.focusKeyword || title} - View ${idx + 1}`;
            changed = true;
          });
          if (changed) handleChange(setImages, newImages);
        }
      } else {
        setAiProgress('Focus Keyword & Image Alts — Failed');
      }
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          ...
          <button className="button secondary" onClick={() => handleSubmit(status === 'DRAFT' ? 'ACTIVE' : status)} disabled={loading}>{loading ? 'Saving...' : 'Save Product'}</button>
        </div>
      </div>
